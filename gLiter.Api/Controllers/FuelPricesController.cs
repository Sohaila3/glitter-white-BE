...
public class FuelPricesController : ControllerBase
{
    ...
    public FuelPricesController(IFuelPriceService fuelPriceService)
    {
    ...
    public async Task<IActionResult> GetAll([FromQuery] string lang = "en", [FromQuery] int pageNumber = 1, [FromQuery] int pageSize = 10)
    {
        ...
        var result = await _fuelPriceService.GetFuelPricesAsync(NormalizeLang(lang), page, size);
        return Ok(result);
    }
    ...
    {
        var result = await _fuelPriceService.GetByIdAsync(id, NormalizeLang(lang));
        return result.Success ? Ok(result) : NotFound(result);
    }

    [HttpPost]
    [Authorize]
    public async Task<IActionResult> Create([FromBody] FuelPriceDto dto)
    {
        ...
        var result = await _fuelPriceService.CreateAsync(dto);
        return Ok(result);
    }

    [HttpPut("{id:int}")]
    [Authorize]
    public async Task<IActionResult> Update(int id, [FromBody] FuelPriceDto dto)
    {
        if (!ModelState.IsValid)
        {
            return BadRequest(ApiResponse<string>.Fail("Invalid input"));
        ...
        var result = await _fuelPriceService.UpdateAsync(id, dto);
        return result.Success ? Ok(result) : NotFound(result);
    }
    ...
    {
        var result = await _fuelPriceService.DeleteAsync(id);
        return Ok(result);
    }
