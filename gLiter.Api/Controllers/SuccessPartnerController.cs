...
public class SuccessPartnerController : ControllerBase
{
    private readonly ISuccessPartnerService _service;
    private const int MaxPageSize = 100;

    public SuccessPartnerController(ISuccessPartnerService service)
    {
        _service = service;
    }
    ...
    public async Task<IActionResult> GetAll([FromQuery] string lang = "en", [FromQuery] int pageNumber = 1, [FromQuery] int pageSize = 10)
    {
        ...
        var result = await _service.GetPartnersAsync(NormalizeLang(lang), page, size);
        return Ok(result);
    }
    ...
    public async Task<IActionResult> Create([FromBody] SuccessPartnerDto dto)
    {
    ...
    [HttpPut("{id:int}")]
    [Authorize(Roles = AdminRoles.SuperAdmin)]
    public async Task<IActionResult> Update(int id, [FromBody] SuccessPartnerDto dto)
    {
        if (!ModelState.IsValid)
        {
            return BadRequest(ApiResponse<string>.Fail("Invalid input"));
